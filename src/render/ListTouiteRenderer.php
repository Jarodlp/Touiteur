...
class ListTouiteRenderer implements Renderer{
    private \iutnc\touiteur\list\ListTouite $list;

    public function __construct(\iutnc\touiteur\list\ListTouite $l){
        $this->list=$l;
    }
            ...
            if($page>1){
                if($this->list->length<=($page*10)){
                    ...
                    for($i=$previousPage*10;$i<$maxIndex+1;$i++){
                        $affichage.=(new \iutnc\touiteur\render\TouiteRenderer($this->list->touites[$i]))->render($selector);
                    }
